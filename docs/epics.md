...
- Signal detection algorithms (subscriptions, credit, savings, income)
...
**Acceptance Criteria:**
1. Subscription detection function created (`backend/app/services/signal_detection.py`)
2. Logic identifies recurring merchants:
   - ≥3 occurrences in 90 days
...
4. Function stores results in `computed_features` table:
   ...
   - `signal_value` = JSON with merchant list and totals
...
**Prerequisites:** Stories 1.4, 3.1 (database schema and seed data exist)

**Technical Notes:**
- Use pandas for data processing
- Implement cadence detection algorithm
- Store results as JSON in signal_value column
- Consider time windows (30d, 90d, 180d)
...
3. Logic detects:
   ...
   - Overdue status (from liability data if available)
4. Function stores results in `computed_features` table:
   - `signal_type` = "credit_utilization_{account_id}"
   - `signal_value` = JSON with utilization %, flags, interest, payment info
5. Function handles:
...
3. Function identifies savings-like accounts (account_type = "savings" or "high_yield_savings")
4. Function stores results in `computed_features` table:
   - `signal_type` = "savings_behavior"
   - `signal_value` = JSON with metrics and flags
5. Function handles:
...
4. Function stores results in `computed_features` table:
   ...
   - `signal_value` = JSON with frequency, variability, buffer
5. Function handles:
...
- Store frequency as enum (weekly, biweekly, etc.)

---

### Story 7.5: Create Compute Features Lambda Function
...
**Acceptance Criteria:**
1. Lambda function created: `compute-features`
2. Function handler in `backend/lambdas/compute_features/handler.py`
3. Function:
   - Receives EventBridge event with `user_id`
   - Fetches user's transactions from database
   - Runs all signal detection functions:
     - Subscription detection
     - Credit utilization detection
     - Savings behavior detection
     - Income stability detection
   - Stores computed features in `computed_features` table
   - Computes for all time windows (30d, 90d, 180d)
4. Function handles errors gracefully (logs to CloudWatch)
5. Function completes within Lambda timeout (5 minutes)
6. Function can be triggered manually or via EventBridge
7. Function is testable

**Prerequisites:** Stories 1.6, 7.1-7.4 (Lambda infrastructure and signal detection functions exist)

**Technical Notes:**
- Use Mangum or similar for Lambda handler
- Connect to RDS database
- Call signal detection functions
- Store results efficiently (batch insert if possible)
- Handle connection pooling for RDS
...
**Acceptance Criteria:**
1. Persona assignment function created (`backend/app/services/persona_assignment.py`)
2. Logic implements hierarchical assignment (priority order):
   ...
   - Persona 2: Subscription-Heavy (priority 2)
     - Criteria: ≥3 recurring merchants AND (monthly recurring ≥$50 OR subscription share ≥10%)
   - Persona 3: Variable Income Budgeter (priority 3)
   ...
   - Default: "General Financial Wellness" (if no criteria met)
3. Function assigns persona per time window (30d, 90d, 180d)
4. Function stores in `persona_assignments` table:
   - `user_id`, `time_window`, `persona`, `assigned_at`
5. Function handles:
...
**Prerequisites:** Stories 1.4, 7.5 (database and computed features exist)
...
### Story 8.2: Create Assign Persona Lambda Function
...
**Acceptance Criteria:**
1. Lambda function created: `assign-persona`
2. Function handler in `backend/lambdas/assign_persona/handler.py`
3. Function:
   - Receives EventBridge event with `user_id`
   - Retrieves computed features for user
   - Runs persona assignment logic
   - Stores persona assignments in database
   - Assigns for all time windows (30d, 90d, 180d)
4. Function triggered after `compute-features` completes
5. Function can also run on schedule (daily refresh)
6. Function handles errors gracefully
7. Function is testable

**Prerequisites:** Stories 1.6, 8.1 (Lambda infrastructure and persona logic exist)

**Technical Notes:**
- Use EventBridge event payload
- Connect to RDS database
- Call persona assignment function
- Store results in persona_assignments table
...
**Acceptance Criteria:**
1. Content catalog structure defined (JSON or database table)
...
6. Content catalog stored in database or JSON file

**Prerequisites:** Story 1.4 (database schema exists)

**Technical Notes:**
- Consider creating `content_catalog` table or using JSON file
- Store rationale templates with variable placeholders
- Include all content from PRD data requirements
- Make content queryable by persona
...
**Acceptance Criteria:**
1. Recommendation generation function created (`backend/app/services/recommendation_engine.py`)
2. Function:
...
**Prerequisites:** Stories 1.4, 8.1, 8.3 (database, persona assignment, and content catalog exist)
...
**Prerequisites:** Story 8.4 (recommendation generation exists)

**Technical Notes:**
- Use Python string formatting or template library
- Extract data from computed_features
...
4. If validation fails:
   ...
   - Or filter prohibited phrases
   - Or flag for operator review
5. Guardrails status stored in decision trace
6. Prohibited phrase list is configurable
7. Function is testable

**Prerequisites:** Story 8.5 (rationale generation exists)

**Technical Notes:**
- Maintain prohibited phrase list (array or config file)
- Use string matching or regex for detection
...
2. Function creates decision trace JSON:
   ...
   ```
3. Function stores trace in `decision_traces` table:
   - `recommendation_id` (foreign key to recommendations)
   - `trace_data` (JSON column)
   - `created_at` (timestamp)
...
**Prerequisites:** Stories 1.4, 8.4 (database and recommendation generation exist)

**Technical Notes:**
- Store trace as JSON in database
- Link trace to recommendation via foreign key
- Include all decision logic details
- Ensure trace completeness

---

### Story 8.8: Create Generate Recommendations Lambda Function
...
**Acceptance Criteria:**
1. Lambda function created: `generate-recommendations`
2. Function handler in `backend/lambdas/generate_recommendations/handler.py`
3. Function:
   - Receives EventBridge event with `user_id`
   - Checks user consent status (skip if not granted)
   - Retrieves user's persona and signals
   - Runs recommendation generation logic
   - Generates rationales
   - Applies tone guardrails
   - Stores recommendations and decision traces
   - Marks recommendations as ready for user
4. Function triggered after `assign-persona` completes
5. Function can also run on schedule (daily refresh)
6. Function handles errors gracefully
7. Function respects consent (doesn't process if consent not granted)
8. Function is testable

**Prerequisites:** Stories 1.6, 8.4-8.7 (Lambda infrastructure and recommendation logic exist)

**Technical Notes:**
- Use EventBridge event payload
- Check consent before processing
- Call recommendation generation function
- Store recommendations and traces
- Handle API timeouts (OpenAI/Claude)
